...
class SparnebelPipeline:

    # Database setup
    conn = sqlite3.connect('sparnebel.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `sparnebel`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM sparnebel WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO sparnebel (date, title, link, content)"
